...
````{r}
data_factor <- group_by(data_filled,wday,interval) %>% summarize (mean=mean(steps))
qplot(x = interval,y=mean,data=data_factor, facets=.~wday,geom = "line")
```

With facets and wordday/weekend
````{r}
data_factor <- group_by(data_filled,typeDay,interval) %>% summarize (mean=mean(steps))
